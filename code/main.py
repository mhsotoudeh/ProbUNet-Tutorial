...
parser.add_argument("--noise_aware", action='store_true', help="If true, will set up a Noise-aware HPU-Net")
parser.add_argument("--forward_model", help="(If Using a Noise-aware HPU-Net) Type of forward model to use (grf_rot / ...)")
parser.add_argument("--noise_ch", type=int, help="(If Using a Noise-aware HPU-Net) 3: Estimate prior_std & noise only / 4: Estimate post_std as well")


# Loss

parser.add_argument("--rec_type", help="Reconstruction Loss Type (mse / loglikelihood / ssim)", required=True)
parser.add_argument("--ll_std_sample_num", type=int, help="(rec_type: loglikelihood) -1: (If Using a Noise-aware HPU-Net) Use post_std as an Estimate for Standard Devation / 0: Directly Predict Standard Devation / 1+: Number of Samples to Estimate Standard Deviation")
...
if args.noise_aware is False:
    ...
    # Set Forward Model
    if args.forward_model == 'grf_rot':
        with open(args.train_file[:args.train_file.rfind(".")] + '_statistics.npy', 'rb') as f:
            stats = np.load(f, allow_pickle=True).item()
            R = torch.tensor(stats['R']).to(device)
        # forward_model = lambda fields : ( R @ fields.reshape(fields.shape[0],s*s).t() ).t().reshape(fields.shape[0],1,s,s)
        forward_model = RotationFM(rotation_matrix=R)
        
    else:
        print('Invalid forward model, exiting...')
        exit()

    model = NoiseAwareHPUNet( forward_model=forward_model, noise_ch=args.noise_ch,
                              in_ch=args.in_ch, out_ch=args.out_ch+extra_out_ch, chs=args.intermediate_ch,
                              latent_num=args.latent_num, latent_channels=args.latent_chs, latent_locks=args.latent_locks,
                              scale_depth=args.scale_depth, kernel_size=args.kernel_size, dilation=args.dilation,
                              padding_mode=args.padding_mode ).double()
...
elif args.rec_type.lower() == 'ssim':
    reconstruction_loss = SSIMLossWrapper(window_size=args.ssim_window_size, max_val=args.ssim_dynamic_range)
